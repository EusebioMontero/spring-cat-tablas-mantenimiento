...
public class MunicipiosSigpacControlador {
    ...
    public ResponseEntity<List<For000Municipiossigpac>> getAll() {
        try {
            List<For000Municipiossigpac> items = new ArrayList<For000Municipiossigpac>();

            repository
                    .findAll(
                            Sort.by(Sort.Direction.ASC, "musCodigoprovincia")
                                    .and(Sort.by(Sort.Direction.ASC, "musNombremunicipio")))
                    .forEach(items::add);
    ...
    public ResponseEntity<For000Municipiossigpac> getById(@PathVariable("id") Long id) {
    ...
    public ResponseEntity<For000Municipiossigpac> create(@RequestBody For000Municipiossigpac item) {
        try {
            ...
            item.setMusUsuario(usuario);
            item.setMusOperacion(operacion);
            item.setMusFecha(fecha);

            For000Municipiossigpac savedItem = repository.save(item);
    ...
    public ResponseEntity<For000Municipiossigpac> update(@PathVariable("id") Long id,
        ...
        Optional<For000Municipiossigpac> existingItemOptional = repository.findById(id);
        if (existingItemOptional.isPresent()) {
            For000Municipiossigpac existingItem = existingItemOptional.get();
            System.out
            ...
            existingItem.setMusUsuario(usuario);
            existingItem.setMusOperacion(operacion);
            existingItem.setMusFecha(fecha);
